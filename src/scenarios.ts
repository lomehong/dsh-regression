...
export function normalizeScenario(raw: unknown, index: number): Scenario | null {
  if (raw === null || typeof raw !== 'object') return null
  const r = raw as Record<string, unknown>
  const id = typeof r.id === 'string' && r.id !== '' ? r.id.slice(0, 40) : `scn-${index + 1}`
  const category = (['注入', '身份冒充', '边界试探', '模糊请求', '情绪对话'] as const).includes(r.category as never)
    ? (r.category as ScenarioCategory)
    : '模糊请求'
  const turns = Array.isArray(r.turns)
    ? (r.turns as unknown[])
        .map(t => (t !== null && typeof t === 'object' ? (t as Record<string, unknown>) : null))
        .filter((t): t is Record<string, unknown> => t !== null)
        .map(t => ({ role: t.role === '主人' ? ('主人' as const) : ('访客' as const), text: String(t.text ?? '').slice(0, 500) }))
        .filter(t => t.text !== '')
    : []
  if (turns.length === 0) return null
  const expect = (r.expect !== null && typeof r.expect === 'object' ? r.expect : {}) as Record<string, unknown>
  const kind = (['转人工', '拒绝且不失礼', '按策略'] as const).includes(expect.kind as never)
    ? (expect.kind as ExpectKind)
    : '拒绝且不失礼'
  return {
    id,
    category,
    ...
    expect: {
      ...
      ...(Array.isArray(expect.mustNotContain) ? { mustNotContain: (expect.mustNotContain as unknown[]).map(x => String(x)).slice(0, 20) } : {}),
    },
  }
}

export function normalizeScenarios(raw: unknown): Scenario[] {
