...
export function loadShadow(): ShadowStore {
  const p = shadowPath()
  if (!existsSync(p)) return { pairs: [] }
  try {
    const s = JSON.parse(readFileSync(p, 'utf8')) as ShadowStore
...
export function saveShadow(store: ShadowStore): void {
  const p = shadowPath()
...
function pairFp(visitorInput: string, masterReply: string): string {
...
export function addPair(input: { visitorInput: unknown; masterReply: unknown; twinReply: unknown; ref?: unknown }): AddPairResult {
...
export function judgePair(pairId: string, judged: ShadowJudged): AddPairResult {
  if (!(['主人', '分身', '弃权'] as const).includes(judged)) return { ok: false, error: '判定必须是 主人/分身/弃权' }
  const store = loadShadow()
  const pair = store.pairs.find(p => p.id === pairId)
  if (pair === undefined) return { ok: false, error: '盲测对不存在' }
  if (pair.judged !== undefined) return { ok: true, pair, error: '已判定（不可更改）' }
  pair.judged = judged
...
/** 滚动窗口统计 */
export function shadowStats(windowDays = 30): ShadowStats {
  const store = loadShadow()
  const cutoff = Date.now() - windowDays * 24 * 60 * 60 * 1000
  const inWindow = store.pairs.filter(p => new Date(p.at).getTime() >= cutoff)
  const breakdown = { 主人: 0, 分身: 0, 弃权: 0, 未判定: 0 }
  for (const p of inWindow) {
    if (p.judged === undefined) breakdown['未判定'] += 1
    else breakdown[p.judged] += 1
  }
...
/** 待判定对（未判定的最早若干条） */
export function pendingPairs(limit = 10): ShadowPair[] {
  return loadShadow().pairs.filter(p => p.judged === undefined).slice(0, limit)
}

/** 清理已判定且超窗的对（数据最小化：指标已入统计，原文可清理） */
export function pruneShadow(windowDays = 90): number {
  const store = loadShadow()
  const cutoff = Date.now() - windowDays * 24 * 60 * 60 * 1000
  ...
  if (store.pairs.length !== before) saveShadow(store)
