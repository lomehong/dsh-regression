...
/** 纯评估：对照期望打分（不含 IO） */
export function evaluate(scenario: Scenario, result: SessionRunResult): EvalResult {
...
export interface RegressionReport {
  ...
  results: EvalResult[]
  /** 失败场景明细（供设置页展示） */
  failures: Array<{ scenarioId: string; category: string; reasons: string[] }>
}
...
export async function runRegression(deps: RegressionRunnerDeps): Promise<RegressionReport> {
...
export function regressionHome(): string {
...
export function saveReport(report: RegressionReport): void {
...
export function allReports(): RegressionReport[] {
  const dir = reportsDir()
  ...
  try {
    for (const name of readdirSync(dir)) {
      ...
      try {
        out.push(JSON.parse(readFileSync(join(dir, name), 'utf8')) as RegressionReport)
      } catch {
        // 单份损坏跳过
      }
    }
